...
export const getWorkspaceMembers = async (workspaceId: string) => {
  try {
    const url = WORKSPACE_URL(workspaceId) + '/members'
...
export const addWorkspaceMembers = async (workspaceId: string, payload: { email: string }) => {
  try {
    const url = WORKSPACE_URL(workspaceId) + '/invite-member'
    const respnose = await axiosAuthFetch('POST', url, payload)
    return respnose?.data
  } catch (error) {
    console.log(error)
  }
